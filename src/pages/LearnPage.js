...
const LearnPage = () => {
  const [title, setTitle] = useState(`What's it for?`);
  const [headline, setHeadline] = useState('Explore the fudamentals of OL, one by one!');
  const [paragraph, setParagraph] = useState('Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.');
  const [explanation, setExplanation] = useState('Click on any element')

  const handleDiagramClick = (title, headline, paragraph) => {
    setTitle(title);
