...
const CircleMenu = ({ position }) => {
  ...
  const circles = [
    { numberOfButtons: 10, radius: 150 }, // Bigger circle
    { numberOfButtons: 7, radius: 100 }   // Smaller circle
  ];
  ...
  useEffect(() => {
    ...
    circles.forEach(circle => {
      ...
      const { x: centerX, y: centerY } = getCenter(position); // Get center based on position
  ...
  const getCenter = (position) => {
    if (position === "center") {
      ...
      return { x: window.innerWidth * 0, y: window.innerHeight * 0.46 }; // Adjust y for better positioning
    } 
  };
  ...
  const containerStyle = {
    ...
    height: '300px',     // Set a height for the containerzIndex: 9999,        // High z-index to avoid interference from other elements
    ...
    <div style={{ ...containerStyle, left: `${getCenter(position).x}px`, top: `${getCenter(position).y}px` }}>
      {buttons.map((button, index) => (
        <div
          ...
          style={{
            ...buttonStyle,
            left: `${button.x - 25}px`, // Adjust position for button size
            top: `${button.y - 25}px`,
            transform: `rotate(${button.rotationAngle}deg)`,
            transformOrigin: 'center',
            width: '50px',      // Set a width for the container
            height: '50px',     // Set a height for the containerzIndex: 9999,        // High z-index to avoid interference from other elements
            backgroundColor: '#e6007e', // Background color for the circle container
        }}
          onClick={() => handleClick(index)}
        >
          <svg viewBox="0 0 120 30" width="50" height="50">
